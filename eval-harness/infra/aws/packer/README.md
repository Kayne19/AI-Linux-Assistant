...
- staged runtime checks that verify service/listener health before the first model-backed probe, then run a verifier command-exec probe over the host path, so setup failures can distinguish gateway boot, provider/model errors, and sandbox-routing regressions
