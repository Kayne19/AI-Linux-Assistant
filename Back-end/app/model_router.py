...
from routing_registry import get_allowed_labels, get_skip_rag_labels
from settings import SETTINGS
from summarizers import ContextSummarizer, HistorySummarizer
...
class ModelRouter:
        ...
        tools = [
            {
                ...
                "description": (
                    ...
                    "Use relevant_documents to scope the search when possible."
                ),
                "parameters": {
                    "type": "object",
        ...
        try:
            if tool_name in {"search_rag_database", "search_RAG_database"}:
                ...
                if query is None or relevant_documents is None:
                    ...
                    result = self.database.retrieve_context(query, relevant_documents)
    ...
    def _should_skip_rag(self, routing_labels):
        ...
        return not routing_labels or any(label in skip_rag_labels for label in routing_labels)
    ...
    def _generate_response(self, turn):
        turn.response = self.responder.call_api(
            turn.user_question,
            turn.retrieved_docs,
            turn.summarized_conversation_history,
            turn.memory_snapshot_text,
    ...
    def _update_history(self, turn):
        ...
        self.update_history("model", turn.response)
    ...
    def _extract_memory(self, turn):
        ...
        try:
            turn.extracted_memory = self.memory_extractor.call_api(turn.user_question, turn.response)
