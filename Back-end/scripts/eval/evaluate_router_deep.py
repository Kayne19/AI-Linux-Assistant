...
def _make_router(settings, shared_state):
    store = InMemoryMemoryStore(project_id="router-deep-eval", shared_state=shared_state)
    return ModelRouter(settings=settings, memory_store=store), store


def run_scenario(settings, scenario):
    shared_state = None
    router, store = _make_router(settings, shared_state)
    ...
    for index, step in enumerate(scenario.get("steps", []), start=1):
        if step.get("restart_before"):
            router, store = _make_router(settings, store.shared_state)
