...
import {
	getStreamStatusAliases,
	getStreamStatusKey,
	getStreamStatusLabel,
} from "./streamStatusText";
import type {
	AsyncState,
	ChatSession,
	Project,
...
export default function App() {
	...
	const projectsReloadedRef = useRef<(projects: Project[]) => void>(
		() => undefined,
	);
	const chatDeletedRef = useRef<(chatId: string) => void>(() => undefined);
	const textDrainCompleteRef = useRef<(chatId: string) => void>(
		() => undefined,
	);
	const councilDrainCompleteRef = useRef<(chatId: string) => void>(
		() => undefined,
	);
	...
	>(() => undefined);
	const runUiStreamStatusUpdaterRef = useRef<
	...
	const council = useCouncilStreaming({
		updateRunUiCouncilEntries: (chatId, updater) =>
			runUiCouncilEntriesUpdaterRef.current(chatId, updater),
	...
	useEffect(() => {
		if (
			!chats.selectedChatId ||
			messages.messagesByChat[chats.selectedChatId]
		) {
			return;
		}
	...
	useEffect(() => {
		council.syncLiveCouncilEntries(
			streaming.selectedRunUi?.councilEntries || [],
			streaming.selectedChatBusy,
		);
	}, [
		council.viewingCouncilMessageId,
		streaming.selectedChatBusy,
		streaming.selectedRunUi?.councilEntries,
	]);
	...
	useEffect(() => {
		...
		council.councilEndRef.current?.scrollIntoView({
			behavior: displayedCouncilEntries.some((entry) => !entry.complete)
				? "auto"
				: "smooth",
			block: "end",
		});
	}, [
		council.councilEndRef,
		council.councilStickToBottomRef,
		displayedCouncilEntries,
	]);
	...
	const canPauseCouncilRun =
		selectedChatRunStatus === "running" &&
		Boolean(streaming.selectedRunUi?.canPauseRun);
	...
	useEffect(() => {
		...
		const intervalId = window.setInterval(() => {
			setStatusAliasIndex(
				(current) => (current + 1) % liveStatusAliases.length,
			);
		}, 2400);
	...
	function handleSelectProject(projectId: string, cachedChats?: ChatSession[]) {
		if (cachedChats) {
			...
			chats.setSelectedChatId((current) =>
				cachedChats.some((chat) => chat.id === current)
					? current
					: cachedChats[0]?.id || "",
			);
		}
	...
	async function handleDeleteProject() {
		const deletingSelectedProject =
			projects.selectedProjectId === projects.editingProjectId;
	...
	if (!projects.selectedProject) {
		...
		composerPlaceholder =
			"Add context and press Enter to resume, or just press Enter to resume without input.";
	...
	const liveStatusSubtext =
		liveStatusAliases[statusAliasIndex] ||
		liveStatusAliases[0] ||
		liveStatusLabel;
			...
			<div className="app-shell" style={appShellStyle}>
				<Sidebar
					...
					onToggleProjectExpansion={(projectId) =>
						projects.setExpandedProjectId((current) =>
				...
				<main className="main-panel">
					...
					<div className="workspace-shell">
						...
						<section
							className={`chat-stage${council.councilActive && !council.councilPanelCollapsed ? " council-open" : ""}`}
						>
							<CouncilPanel
